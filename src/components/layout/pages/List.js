...
export const List = () => {
  ...
  const FecthData = async () => {
    setSkeleton(false);
    await axios
      .get("http://localhost/ReactApps/food-web/UserList.php")
      .then((res) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await axios.get(
      `http://localhost/ReactApps/food-web/SingleUser.php/?email=${form.email}`
    );
  ...
  const handleDelete = async (email) => {
    ...
    if (window.confirm("do u really want to delete ")) {
      const result = await axios.delete(
        `http://localhost/ReactApps/food-web/DeleteUser.php/?email=${email}`
      );
      setData(result.data);
      window.location.reload(false);
    }
  ...
  const handleChangeChk = (e) => {
    ...
    if (name === "Allselect") {
      ...
      let tempUser = currentItems.map((item) =>
        item.email === name ? { ...item, isChecked: checked } : item
      );
  ...
  const deleteSelected = async (e) => {
    ...
    if (window.confirm("Do you really want to delete seleted users?")) {
      ...
      for (let i = 0; i < a.length; i++) {
        const result = await axios.delete(
          `http://localhost/ReactApps/food-web/DeleteUser.php/?email=${a[i].email}`
        );
        // setData(result.data)
  ...
  const DeleteAll = async () => {
    const result = await axios.delete(
      `http://localhost/ReactApps/food-web/DeleteAllUsers.php`
    );
    setData(result.data);
                      ...
                      <ul>
                        ...
                        <li>
                          <Pagination
                            ...
                            numberOfButtons1={numberOfButtons}
                            total={data.length}
                            limit={limit}
                          />
                        </li>
